...
function App() {
  ...
  const submitHandle = (e) => {
    ...
    fetch("http://localhost:3000/create-fake-comments", {
      ...
      body: JSON.stringify({
        ...formData,
        commentCount: parseInt(formData.commentCount),
      }),
    })
      .then((res) => res.json())
      .then((res) => {
  ...
  return (
    <div className="max-w-[1000px] mx-auto py-10">
      <h1 className="text-xl font-bold mb-4">
        Ürünlerle ilgili Fake Yorum Üretici
      </h1>
      ...
      {error && (
        <div className="bg-red-50 text-red-600 p-4 rounded-md">
          Geçersiz bir ürün girdiniz, lütfen doğru bir ürün adı ve sayısı ile
          tekrar deneyin!
        </div>
      )}
      ...
      {comments.length > 0 && (
        <div className="grid gap-y-4">
          {comments.map(({ comment, author }, key) => (
            <section
              key={key}
              className="p-4 rounded border border-zinc-300 mb-4 w-full"
            >
